...
export function _tableVirtualScrollDirectiveStrategyFactory(tableDir: TableItemSizeExDirective) {
...
@Directive({
  selector: 'cdk-virtual-scroll-viewport[tvsItemSizeEx]',
  providers: [{
    provide: VIRTUAL_SCROLL_STRATEGY,
    useFactory: _tableVirtualScrollDirectiveStrategyFactory,
    deps: [forwardRef(() => TableItemSizeExDirective)]
  }]
})
export class TableItemSizeExDirective implements OnChanges, AfterContentInit, OnDestroy {
  private alive = true;
  ...
  private isStickyEnabled(): boolean {
    const nameHeaderRowDefs = '_headerRowDefs';
    return !!this.scrollStrategy.viewport && (this.table[nameHeaderRowDefs] as CdkHeaderRowDef[])
      .map(def => def.sticky)
      .reduce((prevState, state) => prevState && state, true);
  }

  ngAfterContentInit() {
    const nameSwitchDataSource = '_switchDataSource';
    const switchDataSourceOrigin = this.table[nameSwitchDataSource];
    this.table[nameSwitchDataSource] = (dataSource: any) => {
